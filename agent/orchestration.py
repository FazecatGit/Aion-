...
def critique_code(
    ...
    prompt = (
        f"You are a STRICT code reviewer verifying an agent's work.\n\n"
        f"REQUIREMENT: {instruction}\n\n"
        f"ORIGINAL CODE:\n```{lang}\n{original_source[:4000]}\n```\n\n"
        f"MODIFIED CODE:\n```{lang}\n{new_source[:4000]}\n```\n\n"
        f"{test_section}"
        f"{rag_section}"
        f"Evaluate the modified code CRITICALLY:\n"
        f"1. Does it implement the CORRECT function named in the requirement?\n"
        f"   (If the requirement says 'getHappyString', is there a function called 'getHappyString'?)\n"
        f"2. Was the correct function modified/created, or was an UNRELATED function changed by mistake?\n"
        f"3. Does the algorithm actually solve the stated problem correctly?\n"
        f"4. Are there any bugs, off-by-one errors, or missed edge cases?\n"
        f"5. Does it handle the test cases correctly?\n\n"
        f"Be STRICT — if the wrong function was edited, or the algorithm is clearly wrong, say FAIL.\n\n"
