...
class AionClient {
    ...
    constructor() {
        ...
        this.timeout = config.get("timeout", 120_000);
    }
    ...
    async agentEdit(filePath, instruction, sessionId) {
    ...
    async agentApply(filePath, instruction, newSource, sessionId) {
...
async function askCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Aion: Thinking…" }, async () => {
        try {
            ...
            vscode.window.showErrorMessage(`Aion: Could not reach server. Is it running?\n${err.message}`);
        }
    });
}
...
async function editSelectionCommand() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Aion Agent: Working…" }, async () => {
        try {
            ...
            outputChannel.appendLine("");
            // Prompt user
            const action = await vscode.window.showInformationMessage("Aion: Changes ready. Apply them?", "Apply", "Show Diff", "Reject");
            if (action === "Apply") {
            ...
            vscode.window.showErrorMessage(`Aion: Could not reach server.\n${err.message}`);
        }
    });
}
...
async function applyEditCommand() {
    ...
    try {
        const data = await client.agentApply(pendingEdit.filePath, pendingEdit.instruction, pendingEdit.newSource);
        if (data.error || data.status === "error") {
            vscode.window.showErrorMessage(`Aion apply failed: ${data.error}`);
            return;
        }
        // Reload the file in the editor
        const doc = vscode.workspace.textDocuments.find((d) => d.uri.fsPath === pendingEdit.filePath);
        if (doc) {
...
async function showDiff() {
    ...
    const newUri = vscode.Uri.parse(`untitled:${pendingEdit.filePath}.aion-preview`);
    ...
    await vscode.commands.executeCommand("vscode.diff", originalUri, newUri, `Aion: ${pendingEdit.instruction}`);
}
// ── Extension lifecycle ─────────────────────────────────────────────────────
function activate(context) {
    ...
    // Register commands
    context.subscriptions.push(vscode.commands.registerCommand("aion.ask", askCommand), vscode.commands.registerCommand("aion.editSelection", editSelectionCommand), vscode.commands.registerCommand("aion.applyEdit", applyEditCommand));
