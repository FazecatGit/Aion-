...
import Orb from './Orb';
import './index.css';
import { SessionSidebar, OrbDisplay, ToolsPanel, Lightbox, ProcessLogPanel, ImageGenPanel } from './components';
import type { Message, OrbMode, ActiveMode, TestCase, TestResult, ChatSession } from './types';
  ...
  <div
    ...
    >
      <SessionSidebar
        ...
        editingSessionId={editingSessionId} editTitle={editTitle}
        setEditTitle={setEditTitle} setEditingSessionId={setEditingSessionId}
        handleNewChat={handleNewChat} switchSession={switchSession}
        handleRenameSession={handleRenameSession} handleDeleteSession={handleDeleteSession}
      />
      ...
      <ToolsPanel
        ...
        activeMode={activeMode} selectedFilePath={selectedFilePath}
        toolsLoading={toolsLoading} toolsOutput={toolsOutput} runTool={runTool}
      />
      ...
      {activeMode === 'imagegen' && (
        <ImageGenPanel
          setMode={setMode}
          onOpenLightbox={(url) => { setLightboxUrl(url); setLightboxZoom(1); setLightboxPan({ x: 0, y: 0 }); }}
        />
      )}
          ...
          onClick={async () => {
            ...
            const poll = setInterval(async () => {
              try {
                ...
                if (r.ok) { clearInterval(poll); alert('Server restarted successfully!'); }
      ...
      {showProcessPanel && (
        <ProcessLogPanel
          ...
          setExpanded={setProcessExpanded}
      ...
      {lightboxUrl && (
        <Lightbox
          ...
          onClose={() => { setLightboxUrl(null); setLightboxZoom(1); setLightboxPan({ x: 0, y: 0 }); }}
        />
      )}
    </div>
  );
